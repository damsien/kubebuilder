...
func (r *{{ .Resource.Kind }}) SetupWebhookWithManager(mgr ctrl.Manager) error {
	return ctrl.NewWebhookManagedBy(mgr).
		...
		{{- end }}
		{{- if ne .Resource.Webhooks.CustomPath "" }}
		WithCustomPath("{{ .Resource.Webhooks.CustomPath }}").
		{{- end }}
		Complete()
}
{{- else }}
